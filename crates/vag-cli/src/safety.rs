...
/// about one vehicle. What it is doing here: the
...
pub fn spared_notice(spared: &[u16]) -> Option<String> {
	...
	Some(format!(
		"  {} not swept — it is the unit this project has twice damaged, and it \n  \
         reports a fault every time it is read. Name it to read it: \n    \
         vagcan survey --only {}",
		list.join(", "),
		list.join(",")
	))
...
mod tests {
	...
	fn a_whole_car_sweep_spares_the_unit_this_project_has_damaged() {
		// Twice over. The steering assist dropped out a third of
	...
	fn what_was_spared_is_said_out_loud_and_says_how_to_read_it_anyway() {
		...
		assert!(notice.contains("twice damaged"), "{notice}");
