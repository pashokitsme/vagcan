...
pub fn road_force(speed_ms: f64, load: &RoadLoad, c: &Conditions) -> f64 {
	...
	0.5 * c.rho * load.cda * air_speed.abs() * air_speed + c.mass_kg * G * load.crr + c.mass_kg * G * grade_sin(c.grade_percent)
}
...
pub fn power(speed_ms: f64, accel_ms2: f64, engine: Option<EngineState>, load: &RoadLoad, c: &Conditions) -> Power {
...
impl Ratios {
	...
	pub fn learn(engine_rad_s: &Track, speed_ms: &Track, gear: &States) -> Ratios {
		...
		for (label, mut ratios) in seen {
			...
			let agreeing = ratios.iter().filter(|r| ((*r - median) / median).abs() <= SLIP_TOLERANCE).count();
...
mod tests {
	...
	const TYPICAL: Inertias = Inertias {
		wheels_kgm2: 5.5,
		engine_kgm2: 0.34,
	};
	...
	fn the_exact_engine_term_equals_the_ratio_form_while_the_clutch_is_locked() {
		...
		assert!((exact_term - ratio_term).abs() < 1e-6 * ratio_term.abs(), "{exact_term} vs {ratio_term}");
	}
	...
	fn steady_drive() -> (Track, Track, States) {
		let (mut engine, mut speed, mut gear) = (Track::default(), Track::default(), States::default());
		let mut t = 0.0;
