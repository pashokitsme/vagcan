...
pub fn combine(a: &Fit, b: &Fit, _mass_kg: f64, _rho: f64, _delta1: f64) -> Result<RoadLoadResult, Reject> {
	...
	let implied_wind_ms = if cda_sum > 0.0 { (a.cda - b.cda) / cda_sum * mean_speed_ms } else { 0.0 };

	let crr_sum = a.crr + b.crr;
	let crr_percent = if crr_sum > 0.0 {
		100.0 * (a.crr - b.crr).abs() / crr_sum
	} else {
		f64::INFINITY
	};
...
pub fn road_load(passes: &[Fit], mass_kg: f64, rho: f64, delta1: f64) -> Result<RoadLoadResult, Reject> {
	match passes {
...
impl Curve {
	...
	fn valid(&self) -> bool {
		self.vc.is_finite() && self.vc > 0.0 && self.tau.is_finite() && self.tau > 0.0 && self.v0.is_finite() && self.v0 > 0.0
	}
}
...
impl Detector {
	...
	pub fn on_sample(&mut self, t: Seconds, speed_kmh: f64, pedal_pct: Option<f64>, selector: Option<&str>) -> Option<Event> {
		...
		let Some(open) = self.open.as_mut() else {
			...
			let ready = pedal_pct.is_some_and(|p| p <= PEDAL_ZERO_PERCENT) && selector.is_some_and(in_neutral);
...
mod tests {
	...
	const INERTIAS: Inertias = Inertias {
		wheels_kgm2: 5.5,
		engine_kgm2: 0.34,
	};
	...
	const REALISTIC: Sampling = Sampling {
		period_s: 0.05,
		jitter_s: 0.004,
		hold_s: 0.05,
	};
	...
	fn two_reciprocal_passes_cancel_a_grade_and_report_it() {
		...
		assert!((out.implied_grade_percent - 1.0).abs() < 0.01, "grade {}", out.implied_grade_percent);
	...
	fn stream(road: &Road) -> Vec<(Seconds, f64)> {
		...
		pass.speed.t.iter().zip(&pass.speed.v).map(|(t, v)| (*t, v * KMH_PER_MS)).collect()
	}
	...
	fn discarded_when(mut change: impl FnMut(f64, f64, &mut f64, &mut f64, &mut &'static str)) -> Option<&'static str> {
		let mut d = detector();
		for (t, kmh) in stream(&Road::default()) {
	...
	fn a_braking_shaped_deceleration_step_discards_the_pass_and_says_so() {
		...
		for (t, kmh) in stream(&Road::default()) {
			...
			previous_t = Some(t);
			if let Some(Event::Discarded(reason)) = d.on_sample(t, kmh - extra, Some(0.0), Some("N")) {
				why = Some(reason);
				break;
			}
		}
