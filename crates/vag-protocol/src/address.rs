...
const BUILT_IN_SHORT_NUMBERS: &[(u8, u16)] =
    &[(0x01, 0x7E0), (0x02, 0x7E1), (0x09, 0x70E), (0x16, 0x70C), (0x17, 0x714)];
    ...
    match serde_json::from_str::<std::collections::BTreeMap<String, String>>(&text) {
        Ok(map) => map
            ...
            .filter_map(|(number, request)| {
                ...
                Some(UnitNumber { number, request: Some(request), name: None })
            })
            .collect(),
...
fn built_in_pairings() -> Vec<UnitNumber> {
    BUILT_IN_SHORT_NUMBERS
        .iter()
        .map(|&(number, request)| UnitNumber { number, request: Some(request), name: None })
        .collect()
}
...
pub fn parse(text: &str) -> Result<UnitAddress, String> {
    ...
    // unit, not a typo.
    let number = u8::from_str_radix(text, 16)
...
mod tests {
    ...
    fn pairing(number: u8, request: Option<u16>, name: Option<&str>) -> UnitNumber {
        UnitNumber { number, request, name: name.map(str::to_string) }
    }
