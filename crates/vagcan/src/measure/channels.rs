...
use vag_data::catalog::{CatalogStore, MeasurementDef, ReadId, Scaling};
...
fn candidates(store: &CatalogStore, units: &[UnitIdentity]) -> Vec<Candidate> {
    ...
    for unit in units {
        ...
        let emissions = UnitAddress::from_request(request)
            .is_some_and(|address| address.is_emissions_related());
        if emissions {
        ...
        for def in store.for_unit(unit.part_number.as_deref(), unit.odx_name.as_deref()) {
            ...
            match out.iter_mut().find(|c| c.request == request && c.did == did) {
                ...
                None => out.push(Candidate { request, did, def, proven: true }),
            }
        }
    }
    out
...
fn better(a: &Candidate, b: &Candidate, prefer: Prefer) -> std::cmp::Ordering {
    ...
    match prefer {
        Prefer::Finest => quantum(&a.def)
            .total_cmp(&quantum(&b.def))
            .then_with(|| stable(a).cmp(&stable(b))),
...
pub fn resolve(
    ...
    Ok(Set { leading, leading_batch, background, cross_check_speeds })
}
...
mod tests {
    ...
    fn a_channel_is_found_by_the_name_its_catalog_gives_it_and_not_by_identifier() {
        ...
        let set = resolve(&synthetic.store(), &[unit(CLUSTER, "SYN0000001")], false)
    ...
    fn a_store_with_no_speed_channel_says_what_it_looked_for() {
        ...
        let missing = resolve(&synthetic.store(), &[unit(CLUSTER, "SYN0000009")], false)
    ...
    fn without_full_the_barometer_and_the_ambient_sensor_are_not_resolved() {
        ...
        synthetic.write("SYN0000003", vec![
    ...
    fn an_anchored_row_is_not_admitted_because_it_proves_only_one_point() {
        ...
        synthetic.write("SYN0000004", vec![MeasurementDef {
        ...
        let missing = resolve(&synthetic.store(), &[unit(CLUSTER, "SYN0000004")], false)
