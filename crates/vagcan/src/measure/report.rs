...
pub fn recompute(run: &Run, setting: &Setting) -> Derived {
	...
	let top = run.marks.iter().max_by_key(|mark| (mark.to_kmh, std::cmp::Reverse(mark.from_kmh)));
	let cost_mark = top.map(|mark| (mark.from_kmh, mark.to_kmh));
	let accel_at_mark_top = top.and_then(|mark| accel_track.at(mark.closed_at)).unwrap_or(0.0);
...
fn power_series(run: &Run, accel: &Track, model: &Model, window: Seconds) -> (Track, Track) {
	...
	for i in 0..run.samples.engine_speed.len() {
		omega.push(run.samples.engine_speed.t[i], power::omega_from_rpm(run.samples.engine_speed.v[i]));
	}
	...
	for i in 0..speed.len() {
		...
		let engine = match (omega.at(t), omega_dot.at(t), run.samples.gear.at(t)) {
			(Some(w), Some(dot), Some(gear)) if !ratios.slipping(gear, w, v) => Some(EngineState { omega: w, omega_dot: dot }),
			_ => None,
		};
...
fn measured_block(out: &mut String, run: &Run, derived: &Derived, setting: &Setting) {
	...
	let rows: Vec<(String, String, String)> = run
		.marks
		.iter()
		.map(|mark| (format!("{}-{}", mark.from_kmh, mark.to_kmh), mark_time(mark, sigma), avg_accel(mark)))
		.collect();

	let name_w = rows.iter().map(|r| r.0.len()).chain([11]).max().unwrap_or(11);
	let time_w = rows.iter().map(|r| r.1.len()).chain([4]).max().unwrap_or(4);
	let _ = writeln!(out, "    {:<name_w$}   {:<time_w$}   average acceleration", "mark (km/h)", "time");
...
fn computed_block(out: &mut String, run: &Run, derived: &Derived, setting: &Setting) {
	let _ = writeln!(out, "  Run {} — computed   ({})", run.index, conditions(setting, derived));
...
fn conditions(setting: &Setting, derived: &Derived) -> String {
	...
	if let Some(model) = &setting.model {
		...
		parts.push(format!("ρ {:.2} kg/m³ {}", model.conditions.rho, setting.rho_from.as_str()));
...
mod tests {
	...
	fn run() -> Run {
		...
		while t <= 8.0 {
			...
			samples.others.entry("boost actual").or_default().push(t, if t < 0.0 { 1.0 } else { 2.1 });
			t += 0.05;
		}
		Run {
			index: 2,
