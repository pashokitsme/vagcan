...
pub fn write_beside(input: &Path, session: &serde_json::Value) -> Result<PathBuf> {
    ...
    std::fs::write(&out, render(session))
        .with_context(|| format!("writing the chart page to {}", out.display()))?;
    Ok(out)
}
...
mod tests {
    ...
    fn the_embedded_json_round_trips() {
        ...
        let back: serde_json::Value =
            serde_json::from_str(&embedded(&html)).expect("the embedded block is valid JSON");
