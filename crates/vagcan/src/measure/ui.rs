...
pub fn plot(series: &[Series], page: usize, width: u16) -> Option<Plot> {
    ...
    let t0 = drawn
        .iter()
        .filter_map(|i| series[*i].points.t.first().copied())
        .fold(f64::INFINITY, f64::min);
    let t1 = drawn
        .iter()
        .filter_map(|i| series[*i].points.t.last().copied())
    ...
    for (line, i) in lines.iter_mut().zip(drawn) {
        ...
        line.points = (0..source.points.len())
            .map(|n| {
                let v = source.points.v[n];
                let v = match fold {
                    true => y.0 + (v - lo) / (hi - lo) * (y.1 - y.0),
                    false => v,
                };
                (source.points.t[n] - t0, v)
            })
            .collect();
    }

    Some(Plot { axis_unit, y, x: (0.0, t1 - t0), lines, dropped, page: (page, count) })
}
...
///
/// Everything below this line is [`plot`]'s: the page, the fold, the bounds and
/// the key are all decided before a widget is built.
fn draw_chart(frame: &mut Frame, screen: &Screen, area: Rect) {
    let Some(plotted) = plot(&screen.series, screen.chart, area.width) else {
        frame.render_widget(
...
fn draw_plot(frame: &mut Frame, plot: &Plot, area: Rect) {
...
mod tests {
    ...
    fn read(label: &str, unit: &str, points: &[(f64, f64)]) -> Series {
        ...
        Series { label: label.into(), unit: unit.into(), points: track, origin: Origin::Bus }
    }
