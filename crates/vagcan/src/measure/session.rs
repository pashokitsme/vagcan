...
pub enum Event {
    ...
    Degraded { now_hz: f64, was_hz: f64 },
}
...
impl Session {
    ...
    fn on_cycle(&mut self, t: Seconds) -> Option<Event> {
        ...
        Some(Event::Degraded { now_hz: 1.0 / now, was_hz: 1.0 / was })
    }
}
...
mod tests {
    ...
    fn a_collapsed_rate_is_reported_once_and_not_once_per_cycle() {
        ...
        let flags: Vec<&Event> =
            events.iter().filter(|e| matches!(e, Event::Degraded { .. })).collect();
    ...
    fn a_cycle_the_leading_unit_did_not_answer_moves_no_state() {
        ...
        let quiet = SampleSet { engine_speed: Some((0.9, 900.0)), ..SampleSet::default() };
