...
//! them, with no evidence any one existed. `SAFETY.md` names that for what it
//! is: *"An identifier sweep is a fuzz test… a path with a defect in it crashes
//! the server."* It crashed the steering assist on the reference car twice.
//!
...
use vag_data::catalog::{CatalogStore, ReadId};
...
mod tests {
	...
	fn store_with(dir: &std::path::Path, key: &str, dids: &[u16]) -> CatalogStore {
		let catalog = vag_data::catalog::MeasurementCatalog::new(dids.iter().copied().map(def).collect());
	...
	fn a_range_without_a_unit_to_aim_it_at_is_refused() {
		...
		assert!(err.contains("fuzz test"), "it says what it costs, in SAFETY.md's terms: {err}");
