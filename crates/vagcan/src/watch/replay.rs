    ...
    pub fn parse(csv: &str) -> Result<Recording, String> {
        ...
        for line in lines {
            ...
            let values = cells
                .iter()
                .map(|at| {
                    row.get(*at).map(|c| c.trim()).filter(|c| !c.is_empty()).map(str::to_string)
                })
                .collect();
...
pub fn cell_to_bytes(cell: &str, channel: &Channel, raw: bool) -> Option<Vec<u8>> {
    ...
    let count = match &def.scaling {
        ...
        Scaling::Enum { levels } => {
            levels.iter().find(|(_, name)| name == cell).map(|(code, _)| *code as f64)?
        }
...
mod tests {
    ...
    fn a_recording_parses_into_columns_and_samples() {
        ...
        assert_eq!(recording.columns[0], Column { name: "Input shaft speed".into(), raw: false });
        assert_eq!(recording.columns[1], Column { name: "0102".into(), raw: true });
    ...
    fn a_column_headed_by_an_identifier_stays_bytes_even_once_the_row_is_proven() {
        ...
        let mut channels = vec![Channel {
            ...
            def: Some(MeasurementDef {
                ...
                scaling: Scaling::Enum { levels: vec![(5, "D".into())] },
                ..rpm_channel().def.unwrap()
            }),
            ..rpm_channel()
        }];
        let columns = vec![Column { name: "3809".into(), raw: false }];
    ...
    fn a_discrete_state_inverts_by_name_because_that_is_a_lookup_not_a_guess() {
        ...
        let gear = Channel {
            def: Some(MeasurementDef {
                raw_form: RawForm::U8First,
                scaling: Scaling::Enum { levels: vec![(5, "4".into()), (0x0C, "R".into())] },
                ..rpm_channel().def.unwrap()
            }),
            ..rpm_channel()
        };
    ...
    fn columns_resolve_to_channels_by_name_or_by_identifier() {
        let mut channels =
            vec![rpm_channel(), Channel { did: 0x0102, def: None, ..rpm_channel() }];
        let columns = vec![
            Column { name: "Input shaft speed".into(), raw: false },
            Column { name: "0102".into(), raw: true },
            Column { name: "Nothing here".into(), raw: false },
        ];
        let resolved = resolve(&columns, &mut channels, 0x7E1);
    ...
    fn an_unproven_identifier_becomes_a_raw_channel_rather_than_vanishing() {
        ...
        let columns = vec![Column { name: "0306".into(), raw: true }];
        let resolved = resolve(&columns, &mut channels, 0x7E1);
