...
use vag_data_labels::obd::Unconverted;
use vag_uds_client::{EcuIdentity, Reading};
...
pub fn plural(n: usize, singular: &str) -> String {
	if n == 1 { singular.to_string() } else { format!("{singular}s") }
}
...
pub enum SensorLine {
	/// The conversion holds here (see [`vag_data_labels::obd::conversion_for`]).
	Converted(Reading),
...
pub fn render_sensors(unit_label: &str, lines: &[SensorLine]) -> String {
	...
	let refused: Vec<&SensorLine> = lines.iter().filter(|l| matches!(l, SensorLine::Unconverted { .. })).collect();
	if !refused.is_empty() {
		if converted > 0 {
			out.push('\n');
		}
		let off_block = refused.iter().any(|l| {
			matches!(
				l,
