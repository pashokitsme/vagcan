...
pub fn draw(frame: &mut Frame, bars: &[Bar], area: Rect) {
    ...
    let peaks: Vec<String> =
        bars.iter().map(|b| format!("peak {}", Bar::figure(b.peak, b.peak))).collect();
...
mod tests {
    ...
    fn rendered(bars: &[Bar], w: u16, h: u16) -> Vec<String> {
        ...
        (0..h)
            .map(|y| {
                (0..w).map(|x| buffer[(x, y)].symbol().to_string()).collect::<String>()
            })
            .collect()
    }
    ...
    fn before_the_car_has_said_anything_there_is_nothing_to_be_proportional_to() {
        ...
        let bar =
            Bar { label: "power".into(), unit: "kW".into(), value: 0.0, peak: 0.0, origin: Origin::Bus };
    ...
    fn take_is_how_many_the_screen_has_room_for() {
        let all: Vec<Series> = (0..8)
            .map(|i| series(&format!("s{i}"), "", &[1.0, 2.0], Origin::Bus))
            .collect();
