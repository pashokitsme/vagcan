...
impl<T> Sourced<T> {
    ...
    pub fn on(value: T, source: Source, at: impl Into<String>) -> Sourced<T> {
        Sourced { value, source, at: Some(at.into()) }
    }
}
...
impl CarFile {
    ...
    pub fn load(path: &Path) -> anyhow::Result<CarFile> {
        let text = std::fs::read_to_string(path)
            .with_context(|| format!("reading the car file {}", path.display()))?;
    ...
    pub fn road_load(&self) -> Result<(RoadLoad, CarConditions), Vec<&'static str>> {
        ...
        Ok((
            RoadLoad { cda: value(&self.cda), crr: value(&self.crr) },
            CarConditions {
    ...
    fn from_json(text: &str) -> anyhow::Result<CarFile> {
        ...
        let vin =
            root.get("vin").and_then(Value::as_str).context("no `vin` — a car file is keyed by the car")?;
        ...
        if let Some(units) = root.get("units") {
            for unit in units.as_array().context("`units` is not a list")? {
                ...
                let request = u16::from_str_radix(request, 16)
                    .with_context(|| format!("a unit whose `request` is not hexadecimal: {request:?}"))?;
                let part_number =
                    unit.get("part_number").and_then(Value::as_str).context("a unit with no `part_number`")?;
                car.units.push(UnitRef { request, part_number: part_number.to_string() });
            }
        }
        ...
        if let Some(entry) = root.get("tyre") {
            ...
            car.tyre = Some(Sourced { value: value.to_string(), source, at });
        }
            ...
            if entry.get("rho_at_fit").is_none() {
                ...
                car.fit = Some(FitConditions {
                    passes: entry
                        ...
                        .context("`cda` records the conditions of a fit but not how many passes")?
                        as u32,
                    ...
                    grade_estimate_percent: optional_finite(
                        "cda.grade_estimate_percent",
                        entry.get("grade_estimate_percent"),
                    )?,
                });
            }
        }
...
mod tests {
    ...
    fn a_described_car() -> CarFile {
        ...
        car.units.push(UnitRef { request: 0x7E0, part_number: "8V0906264H".into() });
    ...
    fn a_mass_in_running_order_is_not_charged_for_its_driver_twice() {
        ...
        let honest = Mass { running_order_kg: 1395.0, includes_driver: true, aboard_kg: 80.0 };
    ...
    fn a_stated_mass_without_a_driver_gains_exactly_one() {
        ...
        let unladen = Mass { running_order_kg: 1320.0, includes_driver: false, aboard_kg: 80.0 };
        assert_eq!(unladen.total(), 1475.0);
        let running_order = Mass { running_order_kg: 1395.0, includes_driver: true, aboard_kg: 80.0 };
        assert_eq!(unladen.total(), running_order.total());
    }

    #[test]
    fn an_empty_car_is_the_document_and_a_driver_and_nothing_else() {
        assert_eq!(Mass { running_order_kg: 1395.0, includes_driver: true, aboard_kg: 0.0 }.total(), 1395.0);
        assert_eq!(Mass { running_order_kg: 1320.0, includes_driver: false, aboard_kg: 0.0 }.total(), 1395.0);
    }
    ...
    fn a_half_described_car_names_what_it_is_missing_in_words() {
        ...
        car.mass = Some(Sourced::new(
            Mass { running_order_kg: 1395.0, includes_driver: true, aboard_kg: 0.0 },
            Source::Stated,
        ));
