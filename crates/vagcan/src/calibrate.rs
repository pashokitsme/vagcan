...
use crate::analyse::{fit_linear, Thresholds};
use crate::discover::{classify, Behaviour, Column};
...
fn pair(
    ...
    for (t_u, cell) in samples_u {
        ...
        let nearest = samples_r
            ...
            .filter(|(i, _)| !used[*i])
            .min_by(|(_, a), (_, b)| {
                (a.0 - t_u).abs().partial_cmp(&(b.0 - t_u).abs()).unwrap()
            });
...
pub fn calibrate(csv: &str, limits: Thresholds) -> Result<Vec<Calibrated>, String> {
    ...
    if references.is_empty() {
        return Err("no reference column: record at least one measurement the catalog \
    ...
    for unknown in &unknowns {
        ...
        for reference in &references {
            ...
            for &form in FORMS {
                let pairs =
                    pair(unknown, reference, form, samples_u, samples_r, limits.tolerance_s);
...
mod tests {
    ...
    fn recording() -> String {
        let mut csv =
            String::from("t_s,Engine speed_t_s,Engine speed,206F_raw_t_s,206F_raw\n");
        for i in 0..60 {
            let t = i as f64 * 0.1;
