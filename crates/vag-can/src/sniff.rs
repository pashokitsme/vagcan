...
impl IsoTpSniffer {
    ...
    pub fn with_timeout(timeout: Duration) -> Self {
        IsoTpSniffer { in_flight: HashMap::new(), timeout, dropped: 0 }
    }
    ...
    fn consecutive_frame(&mut self, id: u32, data: &[u8], now: Instant) -> Option<SnifferPdu> {
        ...
        if partial.data.len() >= partial.expected {
            ...
            return Some(SnifferPdu { id, data: done.data, frames: done.frames });
        }
        None
    }
    ...
    fn expire(&mut self, now: Instant) {
        ...
        self.in_flight
            .retain(|_, p| now.duration_since(p.last_seen) < timeout);
