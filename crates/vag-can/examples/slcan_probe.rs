...
//! *not* through [`vag_can::SlcanBackend::open`], which would immediately put
...
//! Run:
//! ```text
//! cargo run -p vag-can --features slcan --example slcan_probe -- /dev/cu.usbmodemXXXX
//! ```
...
async fn main() {
    ...
    let script: Vec<Step> = if custom.is_empty() {
        STEPS.iter().map(|(c, w)| Step { cmd: c.to_string(), what: w.to_string() }).collect()
    } else {
        custom
            .iter()
            .map(|c| Step { cmd: format!("{c}\r"), what: "custom".to_string() })
            .collect()
    };
