...
pub enum CapturePayload {
	...
	/// fact has already cost two capture sessions (`.archive/research/labels/rod-labels.md`
	...
	fn markers_round_trip_and_carry_the_wall_clock_anchor() {
		...
		let records = vec![
			CaptureRecord {
				ts_us: 0,
				dir: Direction::Rx,
				payload: CapturePayload::Marker {
					note: wall_clock_anchor(unix_us),
				},
			},
			CaptureRecord {
				ts_us: 5_000_000,
