...
use vag_uds_transport::{CanFrame, RawCanTransport, TransportError};
...
impl RawCanTransport for ReplayCan {
	fn send_frame(&mut self, frame: &CanFrame) -> Result<(), TransportError> {
		match self.queue.front() {
			...
			_ => Err(TransportError::Protocol("replay: send_frame with no matching Tx record".into())),
		}
	}
...
mod tests {
	...
	fn replays_tx_then_rx_in_order() {
		let mut can = ReplayCan::new(vec![tx(0x7E0, vec![0x02, 0x10, 0x03]), rx(0x7E8, vec![0x06, 0x50, 0x03])]);
