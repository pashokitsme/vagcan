...
//!
//! The trait lives here (in `vag-uds-client`, which depends on `vag-data-labels`) rather
//! than in `vag-data-labels`, so the pure data model (`MeasurementDef` and its
//! `interpret`) stays free of any transport/I-O dependency: `vag-data-labels` remains a
//! leaf crate, and the I/O layer reaches down to it.

use vag_data_labels::catalog::{MeasurementCatalog, MeasurementDef, ReadId};
...
/// short response, or a [`vag_data_labels::catalog::Scaling::Anchor`] row observed away
