...
//! (`.archive/research/car/other-ecus.md` §1): eight units were observed answering, each on
...
/// `.archive/research/car/other-ecus.md` §3). So the id half is established either by
...
mod tests {
	...
	fn this_module_knows_nothing_about_label_files() {
		...
		// `vag-data-labels` for measurement catalogs (`read.rs`), so a stray `use`
		...
		assert!(!module.contains("vag_data_labels"), "address.rs must not reach into vag-data-labels");
