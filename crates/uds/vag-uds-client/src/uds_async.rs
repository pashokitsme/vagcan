//! Async UDS client over `vag_uds_transport::AsyncIsoTpTransport`.
//!
...
impl<T: AsyncIsoTpTransport> AsyncUdsClient<T> {
	...
	/// order* — which is the shape the label files store fault names in, and
	...
	pub async fn read_dtc_snapshot(&mut self, code: [u8; 3]) -> Result<Vec<crate::dtc::DtcSnapshot>, UdsError> {
	...
	pub async fn read_dtc_extended(&mut self, code: [u8; 3]) -> Result<Vec<crate::dtc::DtcExtendedData>, UdsError> {
