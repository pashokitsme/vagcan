use vag_uds_capture::{CapturePayload, CaptureRecord, Direction, ReplayCan};
use vag_uds_client::{SoftwareIsoTp, UdsClient};
...
fn rdbi_multiframe_over_replay() {
	...
	let ff = rx(
		0x7E8,
		vec![0x10, 0x14, payload[0], payload[1], payload[2], payload[3], payload[4], payload[5]],
	);
