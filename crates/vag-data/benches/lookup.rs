...
use vag_data::{parse_label, LabelDb, LabelFile};
...
fn synth_corpus() -> Vec<LabelFile> {
    ...
    for i in 0..FILES {
        ...
        for m in 0..MEASUREMENTS_PER_FILE {
            body.push_str(&format!(
                "{:03},{},Measurement {i}-{m},,Range: 0...6500 RPM\n",
                m + 1,
                1
            ));
        }
    ...
    for i in 0..WILDCARD_REDIRECTS {
        index_src.push_str(&format!(
            "REDIRECT,CHAIN.LBL,9{:02}-906-???-AB\n",
            i % 100
        ));
    }
...
fn bench_lookup(c: &mut Criterion) {
    ...
    g.bench_function("resolve_exact_redirect", |b| {
        b.iter(|| black_box(db.resolve(black_box(&exact_pn))))
    });
    g.bench_function("resolve_wildcard_chain", |b| {
        b.iter(|| black_box(db.resolve(black_box(wildcard_pn))))
    });
    g.bench_function("resolve_miss", |b| {
        b.iter(|| black_box(db.resolve(black_box(miss_pn))))
    });
