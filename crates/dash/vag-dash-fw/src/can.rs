//! [`vag_uds_can::CanBackend`] over the ESP32-C3's TWAI controller.
//!
...
//! * **It adds no service.** The UDS allowlist lives in `vag-uds-client` and is
//!   `0x22`, `0x19`, `0x10`, `0x3E`. Nothing here can widen it — this layer
//!   only sees bytes.
//!
//! ## Listen-only is the safe default on a car
//!
//! The read-only rule is why [`TwaiBackend::new`] is not the only constructor.
...
/// `vag-uds-can`'s raw id (bit 31 = 29-bit id, SocketCAN's convention) -> TWAI id.
