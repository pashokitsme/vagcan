...
pub fn slope(track: &Track, index: usize, window: Seconds, scheme: Scheme) -> Option<Slope> {
	...
	Some(Slope {
		a,
		t: t_bar,
		span: track.t[to - 1] - track.t[from],
		sigma,
	})
}
...
pub fn shifts(gear: &States, accel: &[Slope], accel_at_mark_top: f64, window: Seconds) -> Vec<Shift> {
	...
	for (when, from, to) in gear.transitions() {
		...
		let cost_on_mark_s = (accel_at_mark_top > 0.0).then(|| speed_deficit_ms / accel_at_mark_top);
		out.push(Shift {
			t: when,
			from,
			to,
...
mod tests {
	...
	fn a_deficit_smaller_than_the_session_could_resolve_is_not_resolved() {
		...
		let (gear, accel) = lift_trace(false);
		let noisy: Vec<Slope> = accel.iter().map(|s| Slope { sigma: 10.0 * s.sigma, ..*s }).collect();
