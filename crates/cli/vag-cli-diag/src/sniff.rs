...
//! (`.archive/research/labels/rod-labels.md` §4.0a–§4.0c).
//!
...
use vag_uds_can::backend::{CAN_EFF_FLAG, CAN_EFF_MASK};
use vag_uds_can::sniff::{IsoTpSniffer, SnifferPdu};
use vag_uds_capture::{CapturePayload, CaptureRecord, Direction, wall_clock_anchor, write_record};
...
impl<W: Write> SniffSession<W> {
	...
	pub fn new(capture: Option<W>, unix_us: u64, diag_only: bool) -> std::io::Result<Self> {
		let mut session = SniffSession {
			sniffer: IsoTpSniffer::new(),
			capture,
			diag_only,
	...
	pub fn on_frame(&mut self, raw_id: u32, data: &[u8], ts_us: u64) -> std::io::Result<Option<String>> {
...
pub async fn run(device_path: &str, baud: u32, out: Option<&str>, diag_only: bool, seconds: Option<u64>, active: bool) -> anyhow::Result<()> {
	...
	println!(
		...
		if active {
			"NORMAL mode — the adapter will acknowledge frames"
		} else {
			"listen-only"
		},
		match out {
