...
//! (`.archive/research/labels/rod-labels.md` §4.0c).
//!
...
use vag_uds_can::sniff::IsoTpSniffer;
use vag_uds_can::{CAN_EFF_FLAG, CAN_EFF_MASK};
use vag_uds_capture::{CapturePayload, CaptureRecord, parse_wall_clock_anchor};
use vag_uds_client::schedule::split_records;
	...
	for record in records {
		match &record.payload {
			...
			CapturePayload::CanFrame { id, data } => {
				let raw = vag_uds_can::to_raw_id(*id);
...
pub fn to_catalog(fits: &[Fitted]) -> vag_data_labels::catalog::MeasurementCatalog {
	use std::borrow::Cow;
...
pub fn run(capture_path: &str, log_path: &str, out: Option<&str>, limits: Thresholds) -> anyhow::Result<()> {
	...
	let records = vag_uds_capture::read_records(std::io::BufReader::new(capture)).map_err(|e| {
