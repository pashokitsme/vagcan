...
impl Chooser for Scripted {
	fn choose(&mut self, level: &Level<'_>, choices: &[Choice], at: usize) -> Result<Decision> {
		self.seen.push((level.what.to_string(), choices.iter().map(|c| c.name.clone()).collect()));
...
pub fn entries(dir: &Path, level: &Level<'_>) -> Vec<Choice> {
	...
	let mut choices: Vec<Choice> = listing.flatten().filter_map(|entry| describe(&entry.path(), level)).collect();
...
pub fn pick_path(io: &mut impl Chooser, root: &Path, levels: &[Level<'_>]) -> Result<Option<PathBuf>> {
...
fn walk(io: &mut impl Chooser, dir: &Path, level: &Level<'_>, at: &mut usize, from_below: bool) -> Result<Step> {
	let mut first = true;
	loop {
		...
		if first && !from_below && choices.len() == 1 {
			let only = &choices[0];
			io.say(&format!("the only {}: {} ({})", level.what, only.name, only.detail))?;
			*at = 0;
		...
		let aimed = match decision {
			Decision::Take(row) | Decision::Delete(row) | Decision::Copy(row) | Decision::Reveal(row) => match choices.get(row) {
				Some(choice) => {
					*at = row;
					choice.clone()
				}
...
fn to_clipboard(path: &Path) -> Option<&'static str> {
	...
	for (program, args) in [("pbcopy", &[][..]), ("wl-copy", &[]), ("xclip", &["-selection", "clipboard"])] {
...
/// terminal to test.
fn screen(level: &Level<'_>, choices: &[Choice], at: usize, height: u16, width: u16) -> Vec<String> {
	...
	for (row, choice) in choices.iter().enumerate().skip(start).take(count) {
		let text = clip(
			&format!("{} {:<name_width$}  {}", if row == at { "❯" } else { " " }, choice.name, choice.detail),
			width,
		);
	...
	if truncated {
		lines.push(clip(&format!("  … {}-{} of {}", start + 1, start + count, choices.len()), width));
	}
...
mod tests {
	...
	impl TempDir {
		fn new(tag: &str) -> TempDir {
			let path = std::env::temp_dir().join(format!("vagcan-picker-{tag}-{}-{:?}", std::process::id(), std::thread::current().id()));
	...
	fn only_the_kind_that_was_asked_for_is_offered() {
		...
		dir.dir("measures");
		assert_eq!(names(&entries(&dir.0, &Level::files("session").ending(".json"))), ["2026-08-04.json"]);
	...
	fn the_left_arrow_from_the_sessions_offers_the_cars_again() {
		...
		let mut io = Scripted::new([Decision::Take(0), Decision::Back, Decision::Take(1), Decision::Take(0)]);
		let picked = pick_path(&mut io, &cars.0, &car_then_session()).unwrap();
		assert_eq!(picked, Some(cars.0.join("XW8-second/measures/2026-07-19-0902.json")));
	...
	fn the_left_arrow_at_the_first_level_picks_nothing() {
		...
		let picked = pick_path(&mut Scripted::new([Decision::Back]), &cars.0, &car_then_session()).unwrap();
	...
	fn the_highlighted_row_survives_a_trip_up_to_the_cars_and_back_down() {
		...
		let mut io = Scripted::new([Decision::Take(1), Decision::Back, Decision::Take(1), Decision::Take(0)]);
	...
	fn deleting_the_only_session_left_says_so_and_goes_back_up_to_the_cars() {
		...
		let mut io = Scripted::new([Decision::Delete(0), Decision::Delete(0), Decision::Quit]).confirming([true, true]);
		assert_eq!(pick_path(&mut io, &cars.0, &car_then_session()).unwrap(), None);
