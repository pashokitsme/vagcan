...
//! — and [`vag_data_labels::label_files::odx_match`] is the rule that turns those two
...
pub fn open(project: &crate::project::Project) -> Extracted {
	...
	Extracted {
		...
		names: match crate::project::has_source(project, vag_data_db::VCDS) {
...
fn to_def(reading: &vag_data_labels::odis::Reading) -> Option<MeasurementDef> {
...
mod tests {
	...
	fn cache_with(dir: &Path, variants: &[(&str, Vec<vag_data_labels::odis::Reading>)]) -> Extracted {
	...
	fn named_cache_with(dir: &Path, variants: &[(&str, Vec<vag_data_labels::odis::Reading>)], names: &[(&str, &str)]) -> Extracted {
	...
	fn reading(did: u16, name: &str, bit_offset: u32, bit_length: u32, big_endian: bool) -> vag_data_labels::odis::Reading {
		vag_data_labels::odis::Reading {
			did,
			name: name.to_string(),
	...
	fn a_channel_is_named_through_the_text_id_its_own_row_carries() {
		...
		let store = vag_data_labels::catalog::CatalogStore::open(here.path().join("nothing"));
	...
	fn a_proven_row_keeps_the_name_of_whoever_proved_it() {
		...
		let catalog = vag_data_labels::catalog::MeasurementCatalog::new(vec![proven(0x380A, "Input shaft speed")]);
	...
	fn a_project_with_no_extracted_rows_is_the_ordinary_case_and_not_an_error() {
		...
		let store = vag_data_labels::catalog::CatalogStore::open(here.path());
