...
//!
//! Ported from `.archive/research/clb-crack/rod_crack/src/main.rs` (multithreaded Rust
//! brute-forcer) and the framing/prep logic in
//! `.archive/research/clb-crack/rod_struc_decode.py`. Feature-gated (`rod-crack`): it is
//! CPU-heavy and kept out of the default build.
//!
//! Approach (see `.archive/research/labels/rod-labels.md` §1): a `.rod` section is
...
//! `vagcan dev vcds rod <file> --cache <fresh>` end to end went **483.7 s → 83.8 s
...
/// the multi-minute brute force (which the `vagcan dev vcds rod` acceptance run exercises).
#[cfg(test)]
...
///
/// Two regimes, and the difference is expensive (`.archive/research/labels/tttext2.md`):
///
