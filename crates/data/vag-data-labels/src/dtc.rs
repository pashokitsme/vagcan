...
//! `Codes.dat` is keyed by an ISO/SAE DTC. `.archive/research/labels/fault-naming-hop.md` is
...
//! number. `.archive/research/labels/fault-naming-hop.md` has stayed at zero wrong answers and
...
use crate::rod::{IvCache, RodStatus, decode_rod_recover};
...
impl DtcRegistry {
	...
	pub fn parse(text: &str) -> Self {
		...
		for line in text.split("\r\n") {
			...
			rows.push(RowSpan {
				key,
				start: body_at as u32,
				end: (start + line.len()) as u32,
			});
		}
		Self {
			text: text.to_string(),
	...
	pub fn row(&self, index: usize) -> Option<DtcRow<'_>> {
		...
		Some(DtcRow {
			key: span.key,
			payload: &self.text[span.start as usize..span.end as usize],
		})
	}
...
pub fn read_row(row: &DtcRow<'_>) -> Option<FaultName> {
	...
	Some(FaultName {
		codes_key,
		failure_type: alphabet.hex_byte(fields[1]),
	})
}
...
impl UnitCatalogue {
	...
	/// (`.archive/research/labels/fault-naming-hop.md` §10.1), turned into a runtime guard,
