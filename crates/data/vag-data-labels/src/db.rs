...
impl LabelDb {
	...
	pub fn new(files: Vec<LabelFile>) -> Self {
		...
		for (i, f) in files.iter().enumerate() {
			...
			for (ri, r) in f.records.iter().enumerate() {
				match r {
					Record::Redirect {
						target, selector: Some(sel), ..
					} => {
						let mut entry = PreparedRedirect::new(order, target, sel);
						if entry.wildcards == 0 {
							exact_redirects.entry(entry.selector.clone()).or_insert_with(|| entry.target.clone());
						} else {
							wildcard_redirects.entry(entry.selector.len()).or_default().push(entry.clone());
						}
			...
			.find(|f| {
				f.unit.is_some()
					&& f.records.iter().any(|r| match r {
						crate::label::Record::Redirect { target: t, .. } => t.to_ascii_uppercase() == target,
						_ => false,
					})
			})
	...
	fn resolve_idx(&self, part_no: &str) -> Option<usize> {
		...
		{
			let cache = self.resolve_cache.lock().unwrap_or_else(PoisonError::into_inner);
	...
	/// scoped to a single resolved part number. Used by `vagcan dev vcds labels --block`
		...
		let mut hits: Vec<(&str, &Measurement)> = self
			...
			.flat_map(|f| {
				f.records.iter().filter_map(move |r| match r {
					Record::Measurement(m) if m.block == block && field.is_none_or(|wanted| m.field == wanted) && !m.name.trim().is_empty() => {
...
fn collect_unit_numbers(files: &[LabelFile]) -> Vec<(u8, String)> {
	...
	for unit in files.iter().filter_map(|f| f.unit.as_ref()) {
		*counts.entry(unit.address).or_default().entry(unit.name.as_str()).or_default() += 1;
	}
...
fn selector_matches_normalized(selector: &str, pn: &str) -> bool {
	...
	selector.chars().zip(pn.chars()).all(|(s, p)| s == '?' || s == p)
}
...
mod tests {
	...
	fn direct_file_named_after_part_number_resolves() {
		...
		let m = db.measurement("022-906-032-C", 1, 1).expect("measurement should be found");
	...
	fn one_hop_redirect_resolves_to_target() {
		let index = parse_label("INDEX.LBL", b"REDIRECT,TARGET.LBL,022-906-032-C");
		let target = parse_label("TARGET.LBL", b"001,1,Engine Speed,,Range: 0...6500 RPM");
	...
	fn most_specific_selector_wins() {
		let index = parse_label("INDEX.LBL", b"REDIRECT,BASE.LBL,022-906-032\nREDIRECT,EXACT.LBL,022-906-032-C");
	...
	fn same_length_selectors_tiebreak_by_wildcard_count() {
		...
		let index = parse_label("INDEX.LBL", b"REDIRECT,EXACT.LBL,022-906-032-C\nREDIRECT,WILD.LBL,022-906-0??-C");
	...
	fn wildcard_selector_matches_same_length_part_number() {
		let index = parse_label("INDEX.LBL", b"REDIRECT,TARGET.LBL,----???-???-???");
	...
	fn empty_name_placeholder_measurements_are_filtered_from_lookup() {
		let file = parse_label("022-906-032-C.LBL", b"003,1,Vehicle Speed,,Range: 0...300 km/h\n012,1,,,");
		let db = LabelDb::new(vec![file]);
		...
		let real = db.measurement("022-906-032-C", 3, 1).expect("real measurement should be found");
	...
	fn generated_label_files(n: usize) -> Vec<LabelFile> {
		...
		for i in 0..n {
			...
			let body = format!("001,1,Engine Speed {i},,Range: 0...6500 RPM\n007,2,Coolant Temp {i},,Range: -48...143 C\n012,1,,,",);
		...
		files.push(parse_label("CHAIN.LBL", b"REDIRECT,FINAL.LBL,899-906-???-AB\n"));
	...
	fn bulk_lookups_over_generated_label_files_resolve_correctly() {
		...
		for _pass in 0..2 {
			for i in 0..n {
				...
				let m = db.measurement(&pn, 7, 2).unwrap_or_else(|| panic!("{pn} block 7 field 2 must exist"));
