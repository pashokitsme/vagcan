...
//! `vagcan dev vcds dump` binary (which feeds decrypted `.clb` bytes back through
...
fn parse_record(content: &str, comment: Option<&str>) -> Record {
	...
	match tag.as_str() {
		...
		t if t.len() == 4 && t.starts_with('A') && t[1..].chars().all(|c| c.is_ascii_digit()) => Record::Adaptation {
			channel: tag.clone(),
			index: get(1),
			name: get(2),
			location: get(3),
...
mod tests {
	...
	fn the_component_header_names_the_unit_and_its_address() {
		...
		let unit =
			parse_component_comment(" Component: J743 - Transmission Electronic (#02) - 7-Speed Direct Shift Gearbox              (DSG) 0AM").unwrap();
	...
	fn parses_redirect_with_inline_comment() {
		let lf = parse_label("t.lbl", b"REDIRECT,077-910-560-BFMS.CLB,006-410-010-A0  ; BFM (Spyker C8 MY 2005)");
		match &lf.records[0] {
	...
	fn parses_adaptation_and_long_coding() {
		let lf = parse_label("t.lbl", b"A003,1,Engine Speed,, 860...945 RPM\nLC,00,0~7,01,Manufacturer: Audi");
