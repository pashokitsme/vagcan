...
//! supply an ISO DTC. See `.archive/research/labels/codes-dat.md`.
	...
	fn char_of(self, b: u8) -> char {
		...
		match self {
			...
			Self::Windows1251 => match b {
				0xC0..=0xFF => char::from_u32(0x410 + (b as u32 - 0xC0)).expect("in the Cyrillic block"),
...
const CP1252_HIGH: [char; 32] = [
	'€', '\u{FFFD}', '‚', 'ƒ', '„', '…', '†', '‡', 'ˆ', '‰', 'Š', '‹', 'Œ', '\u{FFFD}', 'Ž', '\u{FFFD}', '\u{FFFD}', '\u{2018}', '\u{2019}', '“', '”',
	'•', '–', '—', '˜', '™', 'š', '›', 'œ', '\u{FFFD}', 'ž', 'Ÿ',
];

/// Windows-1251, `0x80..=0xBF` — everything below the contiguous Cyrillic run.
const CP1251_HIGH: [char; 64] = [
	'Ђ', 'Ѓ', '‚', 'ѓ', '„', '…', '†', '‡', '€', '‰', 'Љ', '‹', 'Њ', 'Ќ', 'Ћ', 'Џ', 'ђ', '\u{2018}', '\u{2019}', '“', '”', '•', '–', '—', '\u{FFFD}',
	'™', 'љ', '›', 'њ', 'ќ', 'ћ', 'џ', '\u{00A0}', 'Ў', 'ў', 'Ј', '¤', 'Ґ', '¦', '§', 'Ё', '©', 'Є', '«', '¬', '\u{00AD}', '®', 'Ї', '°', '±', 'І',
	'і', 'ґ', 'µ', '¶', '·', 'ё', '№', 'є', '»', 'ј', 'Ѕ', 'ѕ', 'ї',
];
...
///   (`.archive/research/labels/fault-naming-hop.md` §3). 137 973 is `100 000 + 0x9455` and
...
/// which `.archive/research/labels/codes-dat.md` §3.1 measured on this file: 22 045 `P`,
...
pub fn sae_code(key: u32) -> Option<String> {
	let short = match key {
		...
		k if (SHORT_BAND_START..SHORT_BAND_START + 0x1_0000).contains(&k) => (k - SHORT_BAND_START) as u16,
		_ => return None,
	};
...
pub fn block0_iv(key: u32, file_constant: u8) -> [u8; 8] {
	...
	for (i, slot) in iv.iter_mut().enumerate() {
		let s = seed[i].wrapping_add(KS[(m * (i + 2)) & 0xff]).wrapping_add(file_constant);
...
fn recover_file_constant(records: &[RawRecord]) -> u8 {
	...
	for rec in records {
		...
		if rec.cipher.len() > 8 {
			let tail = tea_cbc_decrypt(&rec.cipher[8..], &key_rod, rec.cipher[..8].try_into().unwrap());
	...
	let logp: Vec<f64> = counts.iter().map(|&n| ((n as f64 + 0.5) / (total as f64 + 128.0)).ln()).collect();
	...
	let firsts: Vec<(u32, usize, [u8; 8])> = records
		...
		.map(|r| {
			...
			(r.key, r.text_len, dec.try_into().expect("one block in, one out"))
		})
		.collect();
...
impl CodesDb {
	...
	pub fn parse_in(data: &[u8], page: CodePage) -> Self {
		...
		let texts = raw
			.into_iter()
			.filter_map(|r| {
				let plain = tea_cbc_decrypt(&r.cipher, &crate::rod::KEY_ROD, block0_iv(r.key, file_constant));
				(r.text_len <= plain.len()).then(|| (r.key, page.decode(&plain[..r.text_len])))
			})
			.collect();
		Self { texts, file_constant }
	}
...
mod tests {
	...
	fn block0_iv_matches_the_vector_recovered_from_the_car() {
		assert_eq!(block0_iv(9_529_586, 0), [0x47, 0x02, 0xc8, 0xcd, 0x6c, 0x50, 0xdc, 0xd3]);
	}
	...
	fn a_key_spells_the_sae_code_vcds_prints() {
		...
		// cars in .archive/research/labels/rd-rod/pairs.tsv — six-digit component keys on the
	...
	fn the_page_reaches_the_stored_text() {
		...
		assert_eq!(CodesDb::parse_in(&file, CodePage::Windows1251).get(9_529_586), Some("Дат"));
