...
pub fn init(spawner: Spawner) {
    ...
    match spawner.spawn(feed_task()) {
        Ok(()) => {
            ...
            esp_println::println!(
                "[health] watchdog armed: {TIMEOUT_SECS} s, fed every {FEED_INTERVAL_MS} ms"
            );
        }
...
fn report_reset() {
    ...
    match reason {
        Some(SocResetReason::SysBrownOut) => {
            warn!(
                "last reset was a BROWN-OUT — the rail collapsed. If this follows a crank, that is 08-power's answer."
            );
        }
        Some(
            ...
            warn!(
                "last reset was the WATCHDOG — the executor stopped scheduling for {TIMEOUT_SECS} s"
            );
        }
        Some(
        ...
        Some(SocResetReason::SysSuperWdt) => {
            warn!(
                "last reset was the SUPER watchdog — the RTC watchdog itself was not being serviced"
            );
        }
        Some(SocResetReason::ChipPowerOn) => {
            ...
            esp_println::println!(
                "[health] power-on (the ROM reports a chip-level brown-out as this too)"
            );
        }
        _ => {}
    }
...
fn locate_panic_slot() {
    ...
    if spare < FlashStorage::SECTOR_SIZE {
        warn!(
            "config partition has no spare sector after the settings — a panic will be printed and then lost"
        );
        return;
    }
    ...
    esp_println::println!(
        "[health] panic slot at 0x{base:06x}, {} bytes spare in the partition",
        spare
    );
}
...
fn report_panic() {
    ...
    if ack == ACK_UNREPORTED {
        ...
        // round.
        if flash
            .write(base + ACK_OFFSET, &ACK_REPORTED.to_le_bytes())
            .is_err()
        {
