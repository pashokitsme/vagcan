...
//!
//! The claim being tested is that the whole protocol stack — `vag-protocol`'s
//! UDS client and allowlist, `vag-can`'s ISO-TP segmentation, `vag-transport`'s
...
//! covered by `vag-can`'s unit tests, which run the identical code on the host.
//!
//! This binary transmits, so it must never be pointed at a car. On a real bus
//! the mode to start from is `TwaiMode::ListenOnly` — see `SAFETY.md` and the
//! module docs of `vag_dash_fw::can`.
...
use log::{error, info};
use vag_can::{CanBackend, IsoTpCan};
