...
Read `.archive/research/car/whole-car-survey.md` §3 first — this file finishes what it started and
...
| Is the missing hop still missing? | **No — closed, and the command ships** — §7 | see `.archive/research/labels/fault-naming-hop.md` §11–§12 |
...
**Step 1 below is closed and the command ships.** `.archive/research/labels/fault-naming-hop.md` found the
...
`crates/cli/vag-cli-diag/src/faults.rs::format_code` already computes the exact key shape
