...
Companion to `.archive/research/labels/rod-labels.md` (§1 for the crypto) and `.archive/research/labels/label-linkage.md`
...
| Is `MUX.rod` blocked the same way? | **No** — classic, and since opened by another pass (`.archive/research/labels/mux.md`); it holds no read identifier, so `TTTEXT2` is now the last file standing | certain (§6.1) |
...
byte (`0x140033a8c`). That is the same global and the same construction `.archive/research/labels/codes-dat.md`
...
Nothing under `.archive/research/clb-crack/` was modified, and the searcher change of §4 was made in a
...
1. ~~**Open `MUX.rod`**~~ — **done by another pass**, `.archive/research/labels/mux.md`. It is the ODX
...
Tooling: `.archive/research/labels/tttext2-sweep/` (Rust driver + `rodread.py` + `textinspect.py`).
