...
`.archive/research/clb-crack/samples/` — never committed. Tooling in `.archive/research/clb-crack/`.
...
- The `.rod` **TEA-CBC is cracked**; the per-record `product`/IV blocker is now **DEFEATED
  ...
  `.archive/research/clb-crack/rod_crack/`; python ref `rod_crack_prep.py`). `STRUC.rod` **fully
...
`.archive/research/clb-crack/decrypt_modern.py::rod_block0_iv`, identical logic):
```
...
### 1.3 Tooling & result
- `.archive/research/clb-crack/rod_struc_decode.py` — pure-Python reference: DFS over the 5 bytes with
...
- `.archive/research/clb-crack/rod_crack/` — standalone multithreaded **Rust** brute-forcer (isolated:
  own empty `[workspace]`, not a workspace member). Header oracle + `miniz_oxide` inflate
  confirmation across all cores.
- `.archive/research/clb-crack/rod_crack_prep.py` — preps `crack_input.bin` for a given file+tag and
...
sidestep STRUC/DOP (tool: `.archive/research/clb-crack/extract_uds.py`, built on the recovered `b8`/`b7`
...
`.archive/research/clb-crack/measure_{series,ttp,final}.py` (the last brute-forces every DID×interpretation×
...
`.archive/research/clb-crack/measure_{coolant,fit,overlay,channels,probe}.py`.
...
Reusable tool: **`.archive/research/clb-crack/rod_name_harvest.py`**.
...
```
Cipher/IV facts cross-checked with `.archive/research/clb-crack/decrypt_modern.py` (`rod_block0_iv`,
...
its evidence are in `.archive/research/car/whole-car-survey.md` §3 and the decoder is
