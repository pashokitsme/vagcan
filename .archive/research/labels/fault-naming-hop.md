...
`.archive/research/labels/codes-dat.md` §4 left one thing between this tool and a named fault: a VAG
...
Read `.archive/research/labels/codes-dat.md` §5 and `.archive/research/car/whole-car-survey.md` §3 first. This file
...
against. It is reproduced in `.archive/research/labels/rd-rod/pairs.tsv`.
...
(`.archive/research/labels/label-linkage.md` §2.4). **The separator is the glyph occurring exactly six
...
The solver is `.archive/research/labels/rd-rod/solve.py`: a DFS over `f0` values longest-first, each
...
Nothing here needs the car. `.archive/research/labels/rd-rod/` holds standalone Python — deliberately not
...
is no longer needed for anything. It stays in `.archive/research/labels/rd-rod/` as the record of how the
...
`.archive/research/labels/rd-rod/alphabet.py` is the derivation, twenty lines, no dependencies. The
...
By this project's own rule (`.archive/research/labels/mux.md`) the writeup ends here and the feature
