...
Companion to `.archive/research/labels/rod-labels.md`. That file records what is already refuted; this one
...
`.archive/research/clb-crack/rod_crack` "does not have the bug". That is wrong. Its bit reader does the
...
> **Update:** superseded by `.archive/research/labels/tttext-codec.md`. `TTTEXT`'s codec was subsequently
...
**established** by `.archive/research/labels/tttext-codec.md` §2 (four records solved blind all match the
...
Analysis was done with throwaway scripts under `/tmp`; nothing was added to `.archive/research/clb-crack`.
