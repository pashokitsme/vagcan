...
3. **A plan cannot sweep.** The steering rack was destroyed by an identifier sweep
   (`SAFETY.md`, `research/eps/eps-j500-report-ru.md`). A device that holds forty
...
- `vag-dash` itself is generic — it takes a plan. A universal version later reads the
...
`vag-dash` exists and draws. Nine frames are in `~/.vagcan/dash/preview/`, regenerated by

```
cargo run -p vag-dash --example panel -- ~/.vagcan/dash/preview
```
