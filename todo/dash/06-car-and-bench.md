...
None of it can be settled from the catalogs. (`05`, which all of it once gated, is done; the
questions that served the sleep and power designs moved to
[`.archive/specs/dash/06-sleep-and-power-questions.md`](../../.archive/specs/dash/06-sleep-and-power-questions.md)
on 2026-09-15.) Read
...
Nothing waits on it any more: the planner batches a unit's due identifiers (8 on the cable, 4
over BLE) and learns "single only" from an NRC `0x13`/`0x14` (`14` §2). The number still says
what rate a page can hold.
...
**Answered 2026-09-13: absolute.** With the engine running at idle the panel read `0.99 bar`
(`research/dash/can-bring-up.md`, the state of 2026-09-13). Open, and the owner's: whether the
panel shows boost absolute or as gauge (`13` D1).
