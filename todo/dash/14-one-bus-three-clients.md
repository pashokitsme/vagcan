...
| 3 | scheduler in `dash`: sources, rates, shared answers | bench | **core done (branch `scheduler-core`); laptop shell done (branch `bus-host`)** (2026-09-14): `vag_uds_client::schedule::Planner`, `no_std`, clock-free; on the laptop `vag-cli-core/src/bus` owns the link, `watch` and `measure` subscribe, every other car command runs through it as a `UnitLink`. Board shell not started. Before it, `can_task` reads every channel of every unit in turn, 50 ms between reads and 200 ms between rounds — ≈2 Hz a channel, ≈9 exchanges/s, visible page or not. To be discussed with the owner; natural to build with 4 |
