...
- **Specified values**: a channel paired with `setpoint` shows how far it is from what its control unit asked for, and a `kind = "drift"` alarm fires when that distance holds. [`docs/dash/dash-toml.md`](docs/dash/dash-toml.md) is the whole file format.
