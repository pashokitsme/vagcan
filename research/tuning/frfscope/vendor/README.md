# research/tuning/frfscope/vendor

Local checkouts of external tools that `frfscope` depends on. Nothing here is
committed (see `.gitignore`) — each is cloned on the machine that needs it.

## VW_Flash

`bri3d/VW_Flash` — used **read-only** by `../frfscope.py` to decrypt/extract Simos18
...
cd research/tuning/frfscope/vendor/VW_Flash && uv sync      # pulls pycryptodome etc.
```
