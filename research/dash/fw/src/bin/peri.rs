...
fn open_settings() -> Settings {
    match Store::open() {
        Ok(mut store) => {
            ...
            match store.load() {
                Ok(config) => {
                    ...
                    Settings { store: Some(store), config, unsaved: false }
                }
                Err(StoreError::Empty) => {
                    info!("nothing stored yet, running on defaults");
                    Settings { store: Some(store), config: Config::default(), unsaved: false }
                }
                Err(e) => {
        ...
        Err(e) => {
            ...
            Settings { store: None, config: Config::default(), unsaved: false }
        }
    }
}
...
async fn button_task(button: Input<'static>, settings: &'static Shared) -> ! {
    ...
    loop {
        ...
        let press = match embassy_futures::select::select(
            Timer::after(Duration::from_millis(DEBOUNCE_MS / 2)),
            REMOTE_PRESS.wait(),
        )
        .await
        {
            embassy_futures::select::Either::First(()) => {
                machine.poll(button.is_low(), embassy_time::Instant::now().as_millis())
            }
        ...
        match press {
            ...
            Some(Press::Long) => {
                info!("[button] held — {}", match visibility() {
...
async fn panel_task(mut usb: esp_hal::usb_serial_jtag::UsbSerialJtagTx<'static, esp_hal::Async>, settings: &'static Shared) -> ! {
    ...
    loop {
        ...
        let compromised =
            report.label_overrun || report.value_overrun || report.unit_dropped || report.value_shrunk || report.glyph_missing;
...
async fn command(settings: &Shared, raw: &[u8]) -> heapless::String<UART_MTU> {
    ...
    match words.next() {
        Some("help") => {
            let _ = write!(
                out,
                "state | get | set brightness N | set page N | save | load | defaults | erase"
            );
        }
